...
import { buildMentorPrompt } from "@/lib/ai/prompt-builder"
import { formatMentorReply } from "@/lib/ai/response-formatter"
...
export async function generateMentorResponse(
  ...
  groqCaller: typeof generateWithGroq = generateWithGroq
): Promise<string> {
  ...
  const prompt = buildMentorPrompt({
    ...
    question,
    context,
    shouldUseAnalytics: !["greeting", "motivation", "explain-concept", "solve-doubt", "general-conversation"].includes(intent),
  })

  return safeCall(async () => {
    const reply = await groqCaller(
      prompt,
      `You are a supportive AI study mentor for engineering entrance preparation. The detected intent is ${intent}. Speak naturally and use the provided context only when relevant.`
    )
